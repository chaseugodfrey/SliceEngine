...
layout (location=2) out float vRandNoise;
...
uniform float uCloudsAmplitude;
uniform float uCloudsIntensity;
uniform float uCloudsSmoothness;

const float cloudsWidthMult = 10.0f;
...
float getWaveHeight(vec2 p, float t) {
	...
	float d2 = pnoise(vec2(p * 5. - t), vec2(1.2, 3.4)) * 0.5 + 0.5;
   	float d3 = pnoise(vec2(p * 5. + t), vec2(0.5, 0.23)) * 0.5 + 0.5;

    return d2 * d3 * uCloudsAmplitude;
}

float getCutoffNum(vec2 p, float t) {

	float d2 = pnoise(vec2(p * 5. - t), vec2(4.5, 6.7)) * 0.5 + 0.5;
   	float d3 = pnoise(vec2(p * 5. + t), vec2(0.5, 0.23)) * 0.5 + 0.5;
...
void main(void){
	...
	vec2 p = vec2(vPos.x + uCloudOffset.x, vPos.z + uCloudOffset.z) * uCloudsSmoothness;
    float t = uTime * uCloudsIntensity;

	float centerHeight = getWaveHeight(p, t);

	vRandNoise = getCutoffNum(p, t);
